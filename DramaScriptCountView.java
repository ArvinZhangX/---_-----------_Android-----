package com.eclipse.snatch.view.countview;
...
import com.eclipse.snatch.R;


/**
 * 描述：不完全自己写的，只是根据countView做优化修改操作 自定义管理线程 与优先级的自定义控件（更加适合用在那些界面多计时器操作哈）
 */
public class DramaScriptCountView extends View {
    private static final String DEFAULT_SUFFIX = ":"; //默认的后缀
    ...
    private int mDay, mHour, mMinute, mSecond, mMillisecond;// 天 时 分 秒 微秒
    private long mRemainTime; // 剩余的时间
    private OnCountdownEndListener mOnCountdownEndListener;// 最后时间的监听
    private OnCountdownIntervalListener mOnCountdownIntervalListener; //时间间隔的监听
    private DramaScriptTimer mDramaScriptTimer; //这个是优化的核心

    private boolean isShowDay, isShowHour,isShowMinute, isShowSecond, isShowMillisecond;//是否显示天、时、分、秒、毫秒
    ...
    public void setlister(OnCountdownEndListener mOnCountdownEndListener){
    	this.mOnCountdownEndListener = mOnCountdownEndListener;
    }
    
    public DramaScriptCountView(Context context) {
        this(context, null);
    }

    public DramaScriptCountView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DramaScriptCountView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.mContext = context;

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.DramaScriptCountView);
        mTimeBgColor = ta.getColor(R.styleable.DramaScriptCountView_timeBgColor, 0xFF444444);
        mTimeBgRadius = ta.getDimension(R.styleable.DramaScriptCountView_timeBgRadius, 0);
        isShowTimeBgDivisionLine = ta.getBoolean(R.styleable.DramaScriptCountView_isShowTimeBgDivisionLine, true);
        mTimeBgDivisionLineColor = ta.getColor(R.styleable.DramaScriptCountView_timeBgDivisionLineColor, Color.parseColor("#30FFFFFF"));
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.DramaScriptCountView_timeBgDivisionLineSize, dp2px(DEFAULT_TIME_BG_DIVISION_LINE_SIZE));
        mTimeBgSize = ta.getDimension(R.styleable.DramaScriptCountView_timeBgSize, 0);

        isTimeTextBold = ta.getBoolean(R.styleable.DramaScriptCountView_isTimeTextBold, false);
        mTimeTextSize = ta.getDimension(R.styleable.DramaScriptCountView_timeTextSize, sp2px(12));
        mTimeTextColor = ta.getColor(R.styleable.DramaScriptCountView_timeTextColor, 0xFF000000);
        isHideTimeBackground = ta.getBoolean(R.styleable.DramaScriptCountView_isHideTimeBackground, true);
        isShowDay = ta.getBoolean(R.styleable.DramaScriptCountView_isShowDay, false);
        isShowHour = ta.getBoolean(R.styleable.DramaScriptCountView_isShowHour, false);
        isShowMinute = ta.getBoolean(R.styleable.DramaScriptCountView_isShowMinute, true);
        isShowSecond = ta.getBoolean(R.styleable.DramaScriptCountView_isShowSecond, true);
        isShowMillisecond = ta.getBoolean(R.styleable.DramaScriptCountView_isShowMillisecond, false);

        mHasSetIsShowDay = ta.hasValue(R.styleable.DramaScriptCountView_isShowDay);
        mHasSetIsShowHour = ta.hasValue(R.styleable.DramaScriptCountView_isShowHour);

        isSuffixTextBold = ta.getBoolean(R.styleable.DramaScriptCountView_isSuffixTextBold, false);
        mSuffixTextSize = ta.getDimension(R.styleable.DramaScriptCountView_suffixTextSize, sp2px(12));
        mSuffixTextColor = ta.getColor(R.styleable.DramaScriptCountView_suffixTextColor, 0xFF000000);
        mSuffix = ta.getString(R.styleable.DramaScriptCountView_suffix);
        mSuffixDay = ta.getString(R.styleable.DramaScriptCountView_suffixDay);
        mSuffixHour = ta.getString(R.styleable.DramaScriptCountView_suffixHour);
        mSuffixMinute = ta.getString(R.styleable.DramaScriptCountView_suffixMinute);
        mSuffixSecond = ta.getString(R.styleable.DramaScriptCountView_suffixSecond);
        mSuffixMillisecond = ta.getString(R.styleable.DramaScriptCountView_suffixMillisecond);
        mSuffixGravity = ta.getInt(R.styleable.DramaScriptCountView_suffixGravity, 1);
        mSuffixLRMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixLRMargin, -1);
        mSuffixDayLeftMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixDayLeftMargin, -1);
        mSuffixDayRightMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixDayRightMargin, -1);
        mSuffixHourLeftMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixHourLeftMargin, -1);
        mSuffixHourRightMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixHourRightMargin, -1);
        mSuffixMinuteLeftMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixMinuteLeftMargin, -1);
        mSuffixMinuteRightMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixMinuteRightMargin, -1);
        mSuffixSecondLeftMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixSecondLeftMargin, -1);
        mSuffixSecondRightMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixSecondRightMargin, -1);
        mSuffixMillisecondLeftMargin = ta.getDimension(R.styleable.DramaScriptCountView_suffixMillisecondLeftMargin, -1);
        ta.recycle();
    ...
    public void start(long millisecond) {
        ...
        if (null != mDramaScriptTimer) {
            mDramaScriptTimer.stop();
            mDramaScriptTimer = null;
        }
        ...
        mDramaScriptTimer = new DramaScriptTimer(millisecond, countDownInterval) {
            @Override
            ...
            public void onFinish() {
                ...
                if (null != mOnCountdownEndListener) {
                    mOnCountdownEndListener.onEnd(DramaScriptCountView.this);
                }
            }
        };
        mDramaScriptTimer.start();
    }
     ...
     */
    public void stop() {
        if (null != mDramaScriptTimer) mDramaScriptTimer.stop();
    }
     ...
     */
    public void pause() {
        if (null != mDramaScriptTimer) mDramaScriptTimer.pause();
    }
     ...
     */
    public void restart() {
        if (null != mDramaScriptTimer) mDramaScriptTimer.restart();
    }
    ...
    public interface OnCountdownEndListener {
        void onEnd(DramaScriptCountView cv);
    }

    public interface OnCountdownIntervalListener {
        void onInterval(DramaScriptCountView cv, long remainTime);
    }
